...
h0s = np.logspace(-1, -10, 10)
z_k_all = [[mp.mpf(0) for _ in h0s] for _ in range(num_k)]

def get_init_m(period, h0=0.0, m0_guess=-0.55):
    np_P, mp_P = float(period), mp.mpf(period)

    def np_tdgl_deriv(m, t):
      return -2*np_a*m - 4*np_b*m**3 + np_h1*np.cos(2*np.pi*t/np_P) + h0*np.cos(4*2*np.pi*t/np_P)

    def mp_tdgl_deriv(t, m):
      return -2*mp_a*m - 4*mp_b*m**3 + mp_h1*mp.cos(2*mp.pi*t/mp_P) + h0*mp.cos(4*2*mp.pi*t/np_P)
...
def get_m_k(m0, period, h0=0):
    def mp_tdgl_deriv(t, m):
      return -2*mp_a*m - 4*mp_b*m**3 + mp_h1*mp.cos(2*mp.pi*t/mp.mpf(period)) + h0*mp.cos(4*2*mp.pi*t/period)
