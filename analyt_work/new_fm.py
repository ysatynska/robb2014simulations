...
def get_init_m(period, h_mult=0.0):
    ...
    def np_tdgl_deriv(m, t):
      return -2*np_a*m - 6*np_b*m**5 + np_h1*np.cos(2*np.pi*t/np_P) + h_mult * (h0 + h2*np.cos(2*2*np.pi*t/np_P) + h4*np.cos(4*2*np.pi*t/np_P))

    def mp_tdgl_deriv(t, m):
      return -2*mp_a*m - 6*mp_b*m**5 + mp_h1*mp.cos(2*mp.pi*t/mp_P) + h_mult * (h0 + h2*mp.cos(2*2*mp.pi*t/mp_P) + h4*mp.cos(4*2*mp.pi*t/mp_P))
...
def get_m_k(m0, period, h_mult = 0):
    def mp_tdgl_deriv(t, m):
      return -2*mp_a*m - 6*mp_b*m**5 + mp_h1*mp.cos(2*mp.pi*t/mp.mpf(period)) + h_mult * (h0 + h2*mp.cos(2*2*mp.pi*t/period) + h4*mp.cos(4*2*mp.pi*t/period))
