...
def np_tdgl_deriv(m1,t1):   # define the TDGL derivative with a sinusoidal applied field
  m = m1
  dmdt = -2*a*m - 4*b*m**3 + h1*np.cos(2*np.pi*t1/P_c)
  return dmdt

def residual(m0):
  # sol = solve_ivp(dm_dt, (0, P_c), m0, t_eval=[P_c])
  sol = odeint(np_tdgl_deriv,m0,np.linspace(0,P_c,100000))
  return sol[-1] - m0
init_guess = -.55
# init_m_Pc = fsolve(residual, init_guess)[0]
init_m_Pc = -0.51175565
...
t_evals_Pc = np.linspace(0,P_c,10000)
# def dm_dt(t, m):
#   return -2 * a * m - 4 * b * m**3 + h1 * np.cos(2 * np.pi * t / P_c)
# sol_c = solve_ivp(dm_dt, (0, P_c), [init_m_Pc], t_eval=t_evals_Pc, dense_output=True)
# m_vals = sol_c.y[0]

def np_tdgl_deriv(t1,m1):
  m = m1
  dmdt = -2*a*m - 4*b*m**3 + h1*mp.cos(2*mp.pi*t1/P_c)
  return dmdt

m_vals = mp.odefun(np_tdgl_deriv,mp.mpf(0.0),init_m_Pc)
...
for k in range(num_k):
    ...
    integrand = mp.quadsubdiv(m0_integrand, [0, P_c]) * exp_term
...
# fft_vals = np.fft.fft(mp.quadsubdiv(m0_integrand, [0, P_c])) / len(mp.quadsubdiv(m0_integrand, [0, P_c]))
# frequencies = np.fft.fftfreq(len(mp.quadsubdiv(m0_integrand, [0, P_c])), d=dt)
...
#     real_part = np.trapz(mp.quadsubdiv(m0_integrand, [0, P_c]) * cos_term, t_evals_Pc)
#     imag_part = np.trapz(mp.quadsubdiv(m0_integrand, [0, P_c]) * sin_term, t_evals_Pc)
