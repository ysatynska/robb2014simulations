...
def np_tdgl_deriv(m1,t1):   # define the TDGL derivative with a sinusoidal applied field (numpy version)
    m = m1
    dmdt = -2*np_a*m - 4*np_b*m**3 + np_h0*mp.cos(2*mp.pi*t1/np_P)
    return dmdt
...
def np_tdgl_deriv_fsolve(m1,t1):   # define the TDGL derivative with a sinusoidal applied field (numpy version)
    m = m1
...
def np_func(m0):                          # define function which returns difference between initial
    ...
    msol= odeint(np_tdgl_deriv_fsolve,m0,t2)           #  (numpy version)
    diff = msol[-1]-m0
...
# np_a = -3 * math.sqrt(3)/4 # material-specific constant
# np_b = 3 * math.sqrt(3)/8 # material-specific constant
np_h0 = 1.5
...
msol= mp.odefun(np_tdgl_deriv,mp.mpf(0.0),-0.51175565)
...
for i in range(0,eps_list_len):
    ...
    m0_soln_np = fsolve(np_func,m0_init)
    print('m0_soln_np = ',m0_soln_np)
    m0_soln_mp = mp.findroot(np_func,[0.98*m0_soln_np[0],1.02*m0_soln_np[0]],tol=1.0e-10,solver = 'bisect')
    print('m0_soln_mp = ',m0_soln_mp)
    msol= mp.odefun(np_tdgl_deriv,mp.mpf(0.0),m0_soln_mp)
...
plt.title("zn2 below Pc",fontsize=20)
plt.xlabel("eps = (Pc-P)/Pc",fontsize=18)
plt.ylabel("zn2",fontsize=18)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.xscale('log')
plt.yscale('log')
plt.plot(eps_list_mp,eps_list_mp, color = 'k', label='eps-sqrt')
for loop in range(0,num_fourier_comps):
    label_string = 'z'+str(loop)+'2'
    plt.plot(eps_list_mp,zn2_mag_array[loop,:], label = label_string)
plt.legend(loc=2)
plt.show()
plt.close()
