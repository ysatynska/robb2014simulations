...
h_mults = np.logspace(-3, -10, num=10)
...
for j, h_mult in enumerate(h_mults):
  ...
  m_k_2 = get_m_k(init_m_c, mp.mpf(P), h_mult)
...
# Title
plt.title(r"Scaling of $z_k$ with Forcing Amplitude $h_{\mathrm{mult}}$", 
          fontsize=15, fontweight="bold")

# Axis labels
plt.xlabel(r"$h_{\mathrm{mult}}$", fontsize=22)
plt.ylabel(r"$z_k$", fontsize=22)
...
plt.plot(
    h_mults, 
    [mp.power(i, mp.mpf(1.0)/mp.mpf(3.0)) for i in h_mults], 
    'k--', lw=1, label=r"$h_{mult}^{1/3}$"
)
