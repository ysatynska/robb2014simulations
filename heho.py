...
def get_init_m(period, h_mult=0.0):
    ...
    def np_tdgl_deriv(m, t):
      return -2*np_a*m - 4*np_b*m**3 + np_h1*np.cos(2*np.pi*t/np_P) + h_mult * (h0 + h2_c*np.cos(2*2*np.pi*t/np_P) + h2_s*np.sin(2*2*np.pi*t/np_P) + h4_c*np.cos(4*2*np.pi*t/np_P) + h4_s*np.sin(4*2*np.pi*t/np_P))

    def mp_tdgl_deriv(t, m):
      return -2*mp_a*m - 4*mp_b*m**3 + mp_h1*mp.cos(2*mp.pi*t/mp_P) + h_mult * (h0 + h2_c*mp.cos(2*2*mp.pi*t/mp_P) + h2_s*mp.sin(2*2*mp.pi*t/mp_P) + h4_c*mp.cos(4*2*mp.pi*t/mp_P) + h4_s*mp.sin(4*2*mp.pi*t/mp_P))
...
def get_m_k(m0, period, h_mult = 0):
    def mp_tdgl_deriv(t, m):
      return -2*mp_a*m - 4*mp_b*m**3 + mp_h1*mp.cos(2*mp.pi*t/mp.mpf(period)) + h_mult * (h0 + h2_c*mp.cos(2*2*mp.pi*t/period) + h2_s*mp.sin(2*2*mp.pi*t/period) + h4_c*mp.cos(4*2*mp.pi*t/period) + h4_s*mp.sin(4*2*mp.pi*t/period))
...
plt.plot(h_mults, [mp.power(i,mp.mpf(1.0)/mp.mpf(3.0)) for i in h_mults], 'k--', lw=1, label=r"$\varepsilon$")
for k in range(num_k):
